...
export const Vortex = (props: VortexProps) => {
  ...
  // Paused flags — mutated without re-render intentionally
  ...
  const draw = (canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D) => {
    // Pause when tab is hidden or canvas is scrolled out of viewport
    if (isHidden.current || isOffscreen.current) {
  ...
  useEffect(() => {
    ...
    // passive: true — never blocks scroll, satisfies client-passive-event-listeners rule
    window.addEventListener("resize", onResize, { passive: true });

    // Pause animation when tab goes to background
